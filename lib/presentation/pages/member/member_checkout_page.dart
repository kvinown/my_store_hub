...
import 'package:my_store_hub/common/point_extension.dart';
...
class MemberCheckoutPage extends StatefulWidget {
  const MemberCheckoutPage({super.key, required this.cart});
...
class _MemberCheckoutPageState extends State<MemberCheckoutPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<ProfileBloc, ProfileState>(builder: (_, state) {
        if (state is ProfileLoaded) {
          ...
          return const Center(
            child: Text(
                'Oopss.. Sepertinya terjadi kesalahan. \nSilahkan Coba Lagi'),
          );
        }
      }),
    );
...
class _ContentMemberCheckout extends StatefulWidget {
  const _ContentMemberCheckout({required this.cart, required this.user});

  final Cart cart;
  final User user;

  @override
  State<_ContentMemberCheckout> createState() => __ContentMemberCheckoutState();
}

class __ContentMemberCheckoutState extends State<_ContentMemberCheckout> {
  final List<String> _paymentMethods = ['TUNAI', 'GOPAY', 'OVO', 'DANA'];
